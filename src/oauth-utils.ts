...
const DEFAULT_MCP_LOGO_URL =
	"https://raw.githubusercontent.com/thoughtspot/mcp-server/refs/heads/main/static/MCP%20Server%20Logo.svg";
const DEFAULT_INSTANCE_LOGO_URL =
	"https://avatars.githubusercontent.com/u/8906680?s=200&v=4";
const DEFAULT_TERMS_URL =
	"https://www.thoughtspot.com/legal/thoughtspot-for-apps";
const DEFAULT_PRIVACY_URL = "https://www.thoughtspot.com/privacy-statement";
const DEFAULT_SIGNUP_URL = "https://www.thoughtspot.com/trial";
...
export function renderApprovalDialog(
	...
	const { serverInfo, state, client } = options;
	const encodedState = btoa(JSON.stringify(state));
	const serverName = sanitizeHtml(serverInfo.name);
	const mcpLogoUrl = serverInfo.mcpLogoUrl ?? DEFAULT_MCP_LOGO_URL;
	const instanceLogoUrl =
		serverInfo.instanceLogoUrl ?? DEFAULT_INSTANCE_LOGO_URL;
	const termsUrl = serverInfo.termsUrl ?? DEFAULT_TERMS_URL;
	const privacyUrl = serverInfo.privacyUrl ?? DEFAULT_PRIVACY_URL;
	const signupUrl = serverInfo.signupUrl ?? DEFAULT_SIGNUP_URL;
	const approvalTitle =
		serverInfo.approvalTitle ??
		`${serverName} wants access<br>to your ThoughtSpot instance`;
      ...
      <html lang="en">
        <head>
          ...
          <style>
            ...
            .approval-form { width: 100%; display: flex; flex-direction: column; align-items: stretch; }
            .form-group { margin-bottom: 28px; }
            .form-group label { display: block; margin-bottom: 8px; font-weight: 400; font-size: 0.94rem; color: #111827; transition: color 0.2s; }
            .form-group label.label-blue { color: #2563eb; }
            .form-group label.label-red { color: #dc2626; }
            .form-group input { width: 100%; padding: 14px 16px; border: 1.5px solid #d1d5db; border-radius: 8px; font-size: 0.94rem; background: #fff; box-sizing: border-box; transition: border-color 0.2s; }
            .form-group input.input-blue { border-color: #2563eb; }
            .form-group input.input-red { border-color: #dc2626; }
            .approval-subtitle { font-weight: 600; font-size: 0.94rem; margin-bottom: 8px; margin-top: 0; }
            .approval-permissions { margin: 0 0 32px 0; padding: 0 0 0 18px; list-style: disc; color: #111827; font-size: 0.94rem; }
            .approval-permissions li { margin-bottom: 8px; line-height: 1.6; }
            .approval-actions { display: flex; justify-content: space-between; gap: 16px; margin-bottom: 18px; }
            .terms-checkbox { margin-bottom: 24px; display: flex; align-items: flex-start; gap: 8px; }
            .terms-checkbox input[type="checkbox"] { margin-top: 3px; }
            .terms-checkbox label { font-size: 0.94rem; line-height: 1.4; color: #111827; }
            .terms-checkbox a { color: #2563eb; text-decoration: none; }
            .terms-checkbox a:hover { text-decoration: underline; }
            .button { flex: 1 1 0; padding: 12px 0; border-radius: 8px; font-weight: 500; font-size: 0.94rem; border: none; cursor: pointer; transition: background 0.2s, color 0.2s; }
            .button-cancel { background: #f3f4f6; color: #6b7280; border: none; }
            .button-cancel:hover { background: #e5e7eb; }
            .button-allow { background: #2563eb; color: #fff; border: none; }
            .button-allow:hover { background: #1a56db; }
            .button-allow:disabled { background: #93c5fd; cursor: not-allowed; }
            .approval-footer { text-align: center; font-size: 0.88rem; color: #111827; margin-top: 8px; }
            .approval-footer a { color: #2563eb; text-decoration: none; margin-left: 0.25em; font-weight: 500; }
            .approval-footer a:hover { text-decoration: underline; }
            @media (max-width: 600px) {
              .approval-card { padding: 18px 4vw 18px 4vw; max-width: 98vw; }
              .approval-logos { gap: 18px; margin-bottom: 18px; }
              .approval-title { font-size: 1.1rem; margin-bottom: 18px; }
            }
          </style>
        </head>
        <body>
          <div class="approval-card">
            <div class="approval-logos">
              ...
              <img src="${instanceLogoUrl}" alt="Instance Logo" class="approval-logo">
            </div>
            ...
            <form class="approval-form" method="post" action="${new URL(request.url).pathname}" id="approvalForm" autocomplete="off" novalidate>
              ...
              <div class="approval-subtitle">${serverName} will be able to:</div>
              ...
              <div class="terms-checkbox">
                ...
                <label for="termsCheckbox">
                  ...
                  and <a href="${privacyUrl}" target="_blank" rel="noopener noreferrer">Privacy Statement</a>.
                </label>
              </div>
          ...
          <script>
            ...
            input.addEventListener('input', function() {
              if (input.value.trim()) { setBlue(); } else { clearColors(); }
              updateAllowButton();
            });
            termsCheckbox.addEventListener('change', updateAllowButton);
            form.addEventListener('submit', function(e) {
              if (!input.value.trim()) {
                e.preventDefault(); setRed(); input.focus();
              } else if (!termsCheckbox.checked) {
                e.preventDefault(); termsCheckbox.focus();
              } else {
                setBlue();
              }
            });
...
export async function parseRedirectApproval(
	...
	let state: { oauthReqInfo?: AuthRequest } & Record<string, unknown>;
...
export function buildSamlRedirectUrl(
	...
	const encodedState = encodeBase64Url(
		new TextEncoder().encode(JSON.stringify(oauthReqInfo)).buffer as ArrayBuffer,
	);
